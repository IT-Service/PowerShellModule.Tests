...
- Added the ability to temporary skip common test for debugging purposes
  ([issue #219](https://github.com/PowerShell/PowerShellModule.Tests/issues/219)).
- Added functionality to Install-ModuleFromPowerShellGallery helper function to allow using Nuget.exe
  when it is already installed in the temp path. Addresses ([issue #301](https://github.com/PowerShell/PowerShellModule.Tests/issues/301)).
...
- Improved Wiki documentation generation to address more example file types
  and increased unit test coverage and function ([issue #256](https://github.com/PowerShell/PowerShellModule.Tests/issues/256)).
...
- Changes to AppVeyor.psm1
  - Updated Invoke-AppveyorInstallTask to provide better informational messages
    ...
    ([issue #293](https://github.com/PowerShell/PowerShellModule.Tests/issues/293)).
...
- Now outputs a warning when a repository does not have a Tests folder
  ([issue #296](https://github.com/PowerShell/PowerShellModule.Tests/issues/296)).
- Update PSSA rule Measure-IfStatement to handle when there is a clause spanning
  ...
  ([issue #238](https://github.com/PowerShell/PowerShellModule.Tests/issues/238)).
- Update call to install PowerShellGet via Install-Module in AppVeyor.psm1 to
  use AllowClobber
  ([issue #310](https://github.com/PowerShell/PowerShellModule.Tests/issues/310)).
- Fix MetaFixers Functions Pipeline Processing.
- Added common opt-in tests for localization ([issue #145](https://github.com/PowerShell/PowerShellModule.Tests/issues/145)).
...
- Added ```PowerShellModule.DocumentationHelper``` modules
...
- Fixed bug in PowerShellModule.DocumentationHelper\MofHelper.psm1 when 'class' mentioned in MOF file outside of header
- Added ability for PowerShellModule.DocumentationHelper\WikiPages.psm1 to display Array type parameters correctly
...
- Added an option to use a markdown lint settings file in the repository which will
  override the default markdown lint settings file in PowerShellModule.Tests repository.
...
- Removed the reference to the function ConvertTo-SpaceIndentation from the warning
  ...
  ConvertTo-SpaceIndentation does not exist anymore ([issue #4](https://github.com/PowerShell/PowerShellModule.Tests/issues/4)).
...
- Changed debug message which outputs the type of the $results variable (in
  AppVeyor.psm1) to use Write-Verbose instead of Write-Info ([issue #99](https://github.com/PowerShell/PowerShellModule.Tests/issues/99)).
- Added `Get-ResourceModulesInConfiguration` to `TestHelper.psm1` to get support installing
  PowerShell modules when testing examples.
...
- Added new common test so that script files (.ps1) are checked for Byte Order
  Mark (BOM) ([issue #160](https://github.com/PowerShell/PowerShellModule.Tests/issues/160)).
  This test is opt-in using .MetaTestOptIn.json.
- Added minimum viable product for applying custom PS Script Analyzer rules to
  check adherence to PowerShell module Kit style guidelines
  ([issue #86](https://github.com/PowerShell/PowerShellModule.Tests/issues/86)).
  The current rules checks the [Parameter()] attribute format is correct in all parameter blocks.
  - Fixed Byte Order Mark (BOM) in files; PowerShellModule.AnalyzerRules.psd1,
  PowerShellModule.AnalyzerRules.psm1 and en-US/PowerShellModule.AnalyzerRules.psd1
  ([issue #169](https://github.com/PowerShell/PowerShellModule.Tests/issues/169)).
  - Extended the parameter custom rule to also validate the Mandatory attribute.
- Fixed so that code coverage can be scan for code even if there is no PowerShellModule
  folder.
...
- Remove the $repoName variable and replace it with $moduleName as it was a
  duplicate.
- Added so that PowerShellModule.Tests is testing it self with it's own common tests
  ([issue #170](https://github.com/PowerShell/PowerShellModule.Tests/issues/170)).
...
- Fixed the markdown test so that node_modules can be deleted when path contains
  an apostrophe ([issue #166](https://github.com/PowerShell/PowerShellModule.Tests/issues/166)).
...
- Code was moved from the example tests into a new helper function Install-DependentModule
  ([issue #168](https://github.com/PowerShell/PowerShellModule.Tests/issues/168)).
...
- Enable so that missing required modules for integration tests is installed if
  running in AppVeyor or show warning if run by user ([issue #168](https://github.com/PowerShell/PowerShellModule.Tests/issues/168)).
- Set registry key HKLM:\Software\Microsoft\PowerShell\DisablePromptToUpdateHelp
  ...
  ([issue #176](https://github.com/PowerShell/PowerShellModule.Tests/issues/176)).
- Added unit tests for helper function Start-PowerShellModuleTests.
...
- Add opt-in parameter RunTestInOrder for the helper function Invoke-AppveyorTestScriptTask
  which enables running integration tests in order ([issue #184](https://github.com/PowerShell/PowerShellModule.Tests/issues/184)).
  - Refactored the class IntegrationTest to be a public sealed class and using
    a named property for OrderNumber
    ([issue #191](https://github.com/PowerShell/PowerShellModule.Tests/issues/191)).
- Add Script Analyzer custom rule to test functions and statements so that
  opening braces are set according to the style guideline ([issue #27](https://github.com/PowerShell/PowerShellModule.Tests/issues/27)).
- When common tests are running on another repository than PowerShellModule.Tests the
  PowerShellModule.Tests unit and integration tests are removed from the list of tests
  to run ([issue #189](https://github.com/PowerShell/PowerShellModule.Tests/issues/189)).
- Fix ModuleVersion number value inserted into manifest in NuGet package produced
  in Invoke-AppveyorAfterTestTask ([issue #193](https://github.com/PowerShell/PowerShellModule.Tests/issues/193)).
- Fix TestRunner.Tests.ps1 to make compatible with Pester 4.0.7 ([issue #196](https://github.com/PowerShell/PowerShellModule.Tests/issues/196)).
- Improved WikiPages.psm1 to include the EmbeddedInstance to the datatype in the
  generated wiki page ([issue #201](https://github.com/PowerShell/PowerShellModule.Tests/issues/201))
...
- Added Invoke-AppveyorTestScriptTask cmdlet functionality for CodeCoverage for Class based resources ([issue #173](https://github.com/PowerShell/PowerShellModule.Tests/issues/173))
...
- Added module PowerShellModule.Container which contain logic to handle the container
...
- Added new opt-in flags to allow enforcement of script analyzer rules ([issue #161](https://github.com/PowerShell/PowerShellModule.Tests/issues/161))
- Updated year in PowerShellModules.Tests.psd1 manifest to 2018.
...
- Changed so that PowerShellModule.Tests repository can analyze code coverage for the
  helper modules ([issue #208](https://github.com/PowerShell/PowerShellModule.Tests/issues/208)).
...
- Changed Get-PSModulePathItem to trim end back slash ([issue #217](https://github.com/PowerShell/PowerShellModule.Tests/issues/217))
...
- Fix codecov no longer generates an error message when uploading test coverage
  ([issue #203](https://github.com/PowerShell/PowerShellModule.Tests/issues/203)).
...
- BREAKING CHANGE: For those repositories that are using parameter `RunTestInOrder`
  ...
  decoration `[Microsoft.PowerShellModuleKit.IntegrationTest(OrderNumber = 1)]` need
...
- BREAKING CHANGE: The parameter `RunInDocker` is removed in helper function
  ...
  using `[Microsoft.PowerShellModuleKit.IntegrationTest()]` or
  `[Microsoft.PowerShellModuleKit.UnitTest()]`, together with the correct named arguments.
...
- Add support for publishing examples configurations to PowerShell Gallery if
  opt-in ([issue #234](https://github.com/PowerShell/PowerShellModule.Tests/issues/234)).
...
- Fix bug in `Invoke-AppveyorAfterTestTask` to prevent Wiki generation function
  ...
  AppVeyor.yml ([issue #245](https://github.com/PowerShell/PowerShellModule.Tests/issues/245)).
- Added support for example and integration test configurations compilation using
  ...
  in configurations ([issue #240](https://github.com/PowerShell/PowerShellModule.Tests/issues/240)).
...
- Added support to Wiki generation for Example files that are formatted in the
  way required by the automatic example publishing code ([issue #247](https://github.com/PowerShell/PowerShellModule.Tests/issues/247)).
...
- Make sure the latest PowerShellGet is installed on the AppVeyor Build Worker
  ([issue #252](https://github.com/PowerShell/PowerShellModule.Tests/issues/252)).
...
- Example publishing can now use a filename without number prefix
  ([issue #254](https://github.com/PowerShell/PowerShellModule.Tests/issues/254)).
...
- Change `Invoke-AppveyorTestScriptTask` to also create a self-signed certificate
  ...
  `Invoke-AppveyorInstallTask` ([issue #255](https://github.com/PowerShell/PowerShellModule.Tests/issues/255)).
- Change `New-DscSelfSignedCertificate` to write information about certificate
  ...
  `Invoke-AppveyorTestScriptTask` does not have to ([issue #259](https://github.com/PowerShell/PowerShellModule.Tests/issues/259)).
- Remove option to use CustomTaskModulePath. It was not used, nor documented. To
  ...
  ([issue #263](https://github.com/PowerShell/PowerShellModule.Tests/issues/263)).
...
- Moved the import of module PowerShellModule.Container to the top of the module file
...
- Fix informational message when publishing examples
  ([issue #261](https://github.com/PowerShell/PowerShellModule.Tests/issues/261)).
- When cloning this repo and checking out the dev branch, the file
  PowerShellModule.AnalyzerRules.Tests.ps1 was always unstaged. This was probably
  due to the .gitattributes file that was introduced in a previous PR.
  EOL in PowerShellModule.AnalyzerRules.Tests.ps1 is now fixed.
- Adding regression tests for
  [issue #70](https://github.com/PowerShell/PowerShellModule.Tests/issues/70)).
- Migrate Pester Test Syntax from v3 -> v4
  ([issue #199](https://github.com/PowerShell/PowerShellModule.Tests/issues/199)).
...
- Removed the default values of the parameter `ExcludeTag` in favor of using
  ...
  ([issue #274](https://github.com/PowerShell/PowerShellModule.Tests/issues/274)).
...
- Removed sections 'Goals' and 'Git and Unicode' as they have become redundant
  ([issue #282](https://github.com/PowerShell/PowerShellModule.Tests/issues/282)).
...
- Added a common tests to test the length of the relative file path so the paths
  ...
  ([issue #188](https://github.com/PowerShell/PowerShellModule.Tests/issues/188)).
- Add new opt-in common test for markdown link linting
  ([issue #211](https://github.com/PowerShell/PowerShellModule.Tests/issues/211)).
- Adding opt-in common test for spellchecking markdown files. Opt-in by
  ...
  .MetaTestOptIn.json ([issue #211](https://github.com/PowerShell/PowerShellModule.Tests/issues/211)).
...
- Move section Phased Meta test Opt-In in the README.md, and renamed it to
  Common Meta test Opt-In ([issue #281](https://github.com/PowerShell/PowerShellModule.Tests/issues/281)).
- Move the change log from README.md to CHANGELOG.md
  ([issue #284](https://github.com/PowerShell/PowerShellModule.Tests/issues/284)).
- Opt-in to the common test ([issue #287](https://github.com/PowerShell/PowerShellModule.Tests/issues/287)).
...
- Change the section "PowerShell Gallery API key" in README.md to to use
  ...
  ([issue #280](https://github.com/PowerShell/PowerShellModule.Tests/issues/280)).
